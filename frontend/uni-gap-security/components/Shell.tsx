...
export function Shell({ children }: { children: React.ReactNode }) {
  ...
  const labReplay = useApp((state) => state.labReplay);
  const startLabReplay = useApp((state) => state.startLabReplay);
  const pauseLabReplay = useApp((state) => state.pauseLabReplay);
  const restartLabReplay = useApp((state) => state.restartLabReplay);
  const setLabReplaySpeed = useApp((state) => state.setLabReplaySpeed);
  ...
  const initials = (user?.name || 'Analyst')
    ...
    .join('')
    .toUpperCase();
  const replayStatus = connection === 'LAB' && labReplay.running ? 'LAB REPLAY — RUNNING' : connection === 'LAB' ? 'LAB REPLAY — PAUSED' : connection === 'LIVE' ? 'LIVE BACKEND' : connection === 'RECONNECTING' ? 'RECONNECTING' : 'BACKEND OFFLINE';
  const replayProgress = labReplay.datasetRows.length ? Math.min(100, Math.round((labReplay.processedRows / Math.max(1, labReplay.datasetRows.length)) * 100)) : 0;
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--bg)' }}>
      <aside className={`fixed z-40 top-0 bottom-0 left-0 w-64 p-4 border-r transition-transform ${mobile ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`} style={{ background: 'var(--surface)', borderColor: 'var(--border)' }}>
        ...
        <div className="absolute bottom-4 left-4 right-4 soft rounded-xl p-3 text-xs"><div className="flex items-center gap-2"><span className="w-2 h-2 rounded-full" style={{ background: connection === 'LAB' ? 'var(--gold)' : 'var(--mint)' }} /><span>One-way ingest active</span></div><div className="mono mt-1" style={{ color: 'var(--muted)' }}>{replayStatus}</div>{connection === 'LAB' && <div className="mt-2 space-y-2"><div className="flex gap-2"><button onClick={startLabReplay} className="soft px-2 py-1 rounded-lg text-[10px]">Start</button><button onClick={pauseLabReplay} className="soft px-2 py-1 rounded-lg text-[10px]">Pause</button><button onClick={restartLabReplay} className="soft px-2 py-1 rounded-lg text-[10px]">Restart</button></div><div className="flex gap-1 flex-wrap">{[1,2,5,10].map((speed) => <button key={speed} onClick={() => setLabReplaySpeed(speed as 1 | 2 | 5 | 10)} className="px-2 py-1 rounded-lg text-[10px]" style={{ background: labReplay.speed === speed ? 'var(--iris)' : 'transparent', color: labReplay.speed === speed ? 'white' : 'var(--muted)' }}>{speed}x</button>)}</div><div className="text-[10px]" style={{ color: 'var(--muted)' }}>7,000-row lab dataset replay · {replayProgress}%</div></div>}</div>
      </aside>
